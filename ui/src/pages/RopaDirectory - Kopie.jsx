...
export default function RopaDirectory() {
  ...
  const [draftItems, setDraftItems] = useState([]); // Import-Ergebnis (Draft)
...
async function loadDirectory(targetOrgId = orgId) {
  setErr('');
  try {
    // immer aus main / DATA_BASE lesen → kein ref-Query
    const r = await fetch(
      `${GW}/api/tenants/${encodeURIComponent(targetOrgId)}/ropa`
    );
    const j = await r.json();
    ...
    // Backend gibt { items: [...] } zurück
    // Wir unterstützen sowohl String-Liste als auch Objekt-Liste
...
async function importAktenplan() {
  ...
  try {
    if (!orgId) throw new Error('Bitte zuerst eine Organisation auswählen.');
    if (!href.trim()) throw new Error('Bitte XDOMEA-URL angeben.');
...
function ProcessCard({ item, orgId, variant, onSspCreated }) {
  const hasSsp = !!item.sspHref;

  async function handleCreateDraft() {
    try {
      // hier ruft ihr euren „SSP anlegen (Draft)“-Endpoint auf,
      // z.B. POST /api/tenants/:orgId/bundles ...
      // Danach:
      if (onSspCreated) {
        await onSspCreated(orgId); // RoPA main neu laden
      }
    } catch (e) {
      // Fehler anzeigen (z.B. per globaler setErr)
    }
  }
        ...
        {hasSsp ? (
          <>
            <Link
              to={`/ssp?org=${encodeURIComponent(
                orgId
              )}&bundle=${encodeURIComponent(item.id.startsWith('bundle-') ? item.id : `bundle-${item.id}`)}`}
              className="text-blue-700 underline text-sm"
            >
              Öffnen
            </Link>
